...
verification_states = {
    ...
    "gaze_direction": {"valid": False, "msg": "✗ Gaze direction unverified"},
...
class GazeVisualization:
    ...
    def _verify_gaze_direction(self, gaze_vector):
        if self.previous_y is not None:
            current_y = self.head_position[1] + gaze_vector[1] * GAZE_LINE_LENGTH
            delta_y = current_y - self.previous_y
            if abs(gaze_vector[1]) > 0.01:
                if gaze_vector[1] > 0 and delta_y <= 0:
                    logger.warning(f"Gaze Y positive ({gaze_vector[1]:.3f}) but not moving up (ΔY: {delta_y:.3f})")
                    return {"valid": False, "msg": "✗ Gaze direction inconsistent (up)"}
                if gaze_vector[1] < 0 and delta_y >= 0:
                    logger.warning(f"Gaze Y negative ({gaze_vector[1]:.3f}) but not moving down (ΔY: {delta_y:.3f})")
                    return {"valid": False, "msg": "✗ Gaze direction inconsistent (down)"}
            self.previous_y = current_y
            return {"valid": True, "msg": "✓ Gaze direction consistent"}
        self.previous_y = self.head_position[1] + gaze_vector[1] * GAZE_LINE_LENGTH
        return {"valid": True, "msg": "✓ Gaze direction initialized"}
    ...
    def update(self, head_position=None, gaze_vector=None, screen_info=None):
        try:
            ...
            if gaze_vector is not None:
                ...
                logger.debug(f"Updated gaze vector: {self.gaze_vector}")
                verification_states["gaze_vector"] = self._verify_gaze_vector(self.gaze_vector)
                verification_states["gaze_direction"] = self._verify_gaze_direction(self.gaze_vector)
                if not verification_states["gaze_direction"]["valid"]:
                    logger.info("Applying fallback correction to gaze vector Y-component.")
                    self.gaze_vector[1] = -self.gaze_vector[1]
            if screen_info is not None:
                ...
                logger.debug(f"Screen info updated: width={self.screen_width}, height={self.screen_height}, distance={self.screen_distance}")
            self._update_head()
            self._update_gaze()
            self._update_screen()
            self._update_text()
        ...
        try:
            ...
            while self.running:
                try:
                    ...
                    logger.debug(f"Received UDP data: {data_str} from {addr}")
                    values = [float(x) for x in data_str.split(',')]
                    if len(values) != 9:
                        logger.error(f"Invalid data length: {len(values)}, expected 9")
                        continue
    ...
    def animate(self, i):
        try:
            if not data_queue.empty():
                ...
                logger.debug("No new data in queue.")
        except queue.Empty:
            pass
        except Exception as e:
